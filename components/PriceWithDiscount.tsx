...
export function PriceWithDiscount({
  ...
  if (!hasDiscount) {
    ...
    return (
      <span className={`font-semibold text-egp-green dark:text-egp-beige ${sizes.main} ${className}`}>
        £{displayTotal}
      </span>
    );
  }
  ...
  if (layout === "stack") {
    return (
      <div className={`flex flex-col gap-1.5 ${alignClass} ${className}`} role="group" aria-label={`Price: was £${originalTotal?.toFixed(2)}, now £${total.toFixed(2)}`}>
        ...
        <span className={`font-bold tracking-tight text-egp-green dark:text-egp-beige ${sizes.main} leading-tight`}>
           £{total.toFixed(2)}
        </span>
      </div>
    );
