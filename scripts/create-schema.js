...
async function createSchema() {
  console.log('🚀 Създаване на схемата в новата база данни...\n');
  
  try {
    // Прилагаме основната схема
    console.log('📝 Създаване на таблици...');
    const { error: schemaError } = await supabase.rpc('exec_sql', { sql_query: schema });
    
    if (schemaError) {
      console.error('❌ Грешка при създаване на таблици:', schemaError);
      return;
    }
    ...
    // Прилагаме triggers
    console.log('📝 Създаване на triggers...');
    const { error: triggersError } = await supabase.rpc('exec_sql', { sql_query: triggers });
    
    if (triggersError) {
      console.error('❌ Грешка при създаване на triggers:', triggersError);
      return;
    }
    
    console.log('✅ Triggers са създадени успешно');
    
    // Прилагаме policies
    console.log('📝 Създаване на policies...');
    const { error: policiesError } = await supabase.rpc('exec_sql', { sql_query: policies });
    
    if (policiesError) {
      console.error('❌ Грешка при създаване на policies:', policiesError);
      return;
    }
    
    console.log('✅ Policies са създадени успешно');
    
    console.log('\n🎉 Схемата е създадена успешно!');
    console.log('📋 Всички таблици са готови за използване.');
