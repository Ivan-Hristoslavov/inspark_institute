...
async function createTables() {
  console.log('🚀 Създаване на таблици в новата база данни...\n');
  
  try {
    // Проверяваме дали таблицата admin_profile съществува
    ...
    if (checkError) {
      console.log('📋 Информация: Не можем да проверим съществуващите таблици, но ще продължим...');
    } else {
      console.log('📋 Намерени таблици:', existingTables?.map(t => t.table_name) || []);
    }
    
    // Опитваме се да създадем admin_profile таблица
    console.log('📝 Създаване на admin_profile таблица...');
    ...
    if (adminProfileError && adminProfileError.code === 'PGRST116') {
      console.log('⚠️  Таблицата admin_profile не съществува, но не можем да я създадем с JS клиент');
    } else {
      console.log('✅ Таблицата admin_profile вече съществува');
    }
    
    // Опитваме се да създадем customers таблица
    console.log('📝 Проверяване на customers таблица...');
    ...
    if (customersError && customersError.code === 'PGRST116') {
      console.log('⚠️  Таблицата customers не съществува');
    } else {
      console.log('✅ Таблицата customers вече съществува');
    }
    
    // Опитваме се да създадем bookings таблица
    console.log('📝 Проверяване на bookings таблица...');
    ...
    if (bookingsError && bookingsError.code === 'PGRST116') {
      console.log('⚠️  Таблицата bookings не съществува');
    } else {
      console.log('✅ Таблицата bookings вече съществува');
    }
    
    console.log('\n📋 ИНСТРУКЦИИ ЗА СЪЗДАВАНЕ НА ТАБЛИЦИ:');
    console.log('Тъй като не можем да създадем таблици с JS клиент, моля:');
    console.log('1. Отидете в Supabase Dashboard');
    console.log('2. Отидете в SQL Editor');
    console.log('3. Изпълнете следния SQL код:');
    console.log('\n' + '='.repeat(50));
...
`;
    ...
    console.log('\n4. След изпълнение на SQL-а, всички таблици ще бъдат създадени');
    console.log('5. Можете да проверите в Table Editor дали таблиците са създадени');
