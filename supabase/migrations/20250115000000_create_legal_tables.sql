-- Create legal tables for admin panel
-- This migration creates 3 tables for managing legal content
...
DROP TRIGGER IF EXISTS update_site_guidance_updated_at ON site_guidance;
...
-- Insert default terms content (only if table is empty)
...
-- Insert default privacy policy content (only if table is empty)
...
-- Insert default site guidance content (only if table is empty)
INSERT INTO site_guidance (title, content, category, sort_order) 
SELECT * FROM (VALUES 
...
) AS guidance_data(title, content, category, sort_order)
WHERE NOT EXISTS (SELECT 1 FROM site_guidance); 